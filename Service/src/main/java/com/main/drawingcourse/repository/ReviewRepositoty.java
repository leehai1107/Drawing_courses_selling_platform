...
public interface ReviewRepositoty extends JpaRepository<Review, Integer> {
    @Query(
            value = "SELECT r.* FROM Courses c INNER JOIN course_order co ON c.course_id = co.course_id INNER JOIN Reviews r ON co.id = r.id WHERE co.order_id IN (SELECT order_id FROM orders WHERE user_id = :id)",
            nativeQuery = true)
