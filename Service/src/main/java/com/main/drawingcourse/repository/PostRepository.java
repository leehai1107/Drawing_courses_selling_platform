...
//    List<PostModel> findPostByPostcategoryId(int id);
	@Query(value = "SELECT * FROM POSTS p WHERE p.post_category_id = :id", nativeQuery = true)
	...
	@Modifying
	@Query(value = "DELETE p FROM POSTS p WHERE p.post_id = :id AND p.user_id IN ( SELECT user_id FROM users u WHERE u.role_id IN ( SELECT role_id FROM roles r WHERE r.role_id = 1 or r.role_id = 2 or r.role_id = 4))", nativeQuery = true)
	void deleteByPostId(@Param("id") int id);

	@Query(value = "SELECT * FROM POSTS p WHERE p.status = 1", nativeQuery = true)
	List<Post> viewpostbystatustrue(@Param("status") boolean status);

	@Query(value = "SELECT * FROM POSTS p where p.post_id = :id", nativeQuery = true)
